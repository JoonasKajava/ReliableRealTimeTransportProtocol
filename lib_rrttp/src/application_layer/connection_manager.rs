...
pub struct ConnectionManager {
    message_queue: SyncSender<Vec<u8>>,
}
...
impl ConnectionManager {
    pub fn start(local_addr: &str) -> std::io::Result<(Self, Receiver<ConnectionEventType>)> {
        ...
        thread::spawn(move || {
            ...
            loop {
                receiving_window.shift_window().into_iter().for_each(|frame| {
                    ...
                    connection_events_sender_receiver.send(ConnectionEventType::ReceivedFrame(frame)).unwrap();

                    if control_bits.contains(ControlBits::EOM) {
                        let message: Vec<u8> = std::mem::take(&mut buffer);
                        connection_events_sender_receiver.send(ConnectionEventType::ReceivedCompleteMessage(message)).unwrap();
                    }
                });
        ...
        thread::spawn(move || {
            let mut transmitter_window: TransmitterWindow = TransmitterWindow::new(ack_receiver, connection_events_sender_transmitter.clone(), sender_socket);
            loop {
                let next_message: Vec<u8> = messages_to_send_receiver.recv().unwrap();

                transmitter_window.send_message(next_message);
                connection_events_sender_transmitter.send(ConnectionEventType::SentMessage).unwrap();
            }
        });
        Ok((Self {
            message_queue: messages_to_send
