...
%Tietoliikenneprotokollan kehitys UDP:n laajennuksena
...
\setlength{\tabcolsep}{0cm}
        ...
        Tässä insinöörityössä esitetään tietoliikenneprotokollan kehitys UDP:n päälle, minkä nimeksi tuli RRTP-protokolla. Protokollaa varten tehtiin ohjelmakirjasto Rust-kielelle, josta lopputuloksena syntyi RRTP-protokollan toteuttava ohjelmakirjasto. Se yhdistää liukuvan ikkunan ja muita ominaisuuksia luotettavan yhteyden muodostamiseen. \par

        Protokollan kestävyys testattiin demosovelluksella, joka kehitettiin tässä työssä käyttämällä Tauri-ohjelmistokehystä. Tauri-ohjelmistokehys suorittaa käyttöliittymän piirtämisen. Sovellus hyödyntää RRTP-ohjelmakirjastoa yhteyden muodostukseen ja tiedon välittämiseen kahden tietokoneen välillä. \par
        ...
        Lopuksi pohdittiin työtapoja ja työkaluja, joita käytettiin kehityksessä. Työssä käytettiin Git-versionhallintaa ja kehitysympäristö koostui Windows-tietokoneesta sekä RustRover:sta, joka on Rust-kieleen erikoistunut ohjelmointiympäristö. 
...
pub struct Frame {
    ...
    Lähdekoodissa \ref{lst:set_seq_num} järjestysnumero muutetaan laskevaan tavujärjestykseen käyttäen Rust-standardikirjaston \lstinline{to_be_bytes()} funktiota \cite[\textit{u32}]{rust-std}. Tämän jälkeen tulos kopioidaan paketin ensimmäisen 4 tavun tilalle. \par
...
\lstinline{window_size}-kenttä on aina $w_s$ kokoinen. Ikkunan kokoa on mahdollista muuttaa käyttämällä \lstinline{set_window_size}-funktiota, joka muuttaa tämän kentän arvoa ja muuttaa \lstinline{frame_status} taulukon kokoa.\par
...
pub fn shift_window(&mut self) -> Vec<Frame> {
    ...
    Jokainen valmis paketti käydään läpi ja sen sisältö lisätään ylemmän kerroksen puskuriin, jonka jälkeen ohjausbitit luetaan paketista kappaleen \ref{subsec:control_bits} mukaisesti. Vastaanotetusta paketista lähetetään tieto ylempiin kerroksiin (kuten sovellus), jotka voivat käyttää tätä tietoa esim. latausindikaattorin tekemisessä. \par
...
pub struct TransmitterWindow {
    ...
    Tässä toteutuksessa käytetään valikoivaa toistoa, sillä se on tehokas tiedonsiirron kannalta, mutta se vaatii datan puskuroinnin. Puskurointi vaatii huomattavaa muistin käyttöä, jota ei mahdollisesti ole pienissä laitteissa. Tämä toteutus on tehty sillä oletuksella, että alustana on tietokone ja muistista ei ole pulaa.\par
    ...
    \begin{lstlisting}[caption={JSON-data}, label={lst:json_data}]
{
    ...
    Tässä toteutuksessa on käytetty \lstinline{bincode} nimistä kirjastoa binääridatan muodostukseen.
...
fn calculate_checksum(data: &[u8]) -> u16 {
    ...
    Rust-ohjelmointikielessä kokonaisluvun ylivuoto on määritelty. Ylivuoto johtaa aina ohjelman paniikkiin, joka Rust-ohjelmointikielessä viittaa virheeseen, josta ei voi palautua. Paniikki johtaa aina prosessin sammumiseen \cite[luku 9.3]{rust-book}. Tämä käytännössä eliminoi ylivuodon aiheuttamat tietoturvaongelmat \cite[luku 3.2]{rust-book}.
...
fn checksum() {
    ...
    Demosovellus on rakennettu käyttäen Tauri-ohjelmistokehystä.
    Tauri on Electronin kaltainen ohjelmistokehys, joka pyörii Node.js runtime ympäristön päällä ja tarjoaa version Chromium selaimesta käyttöliittymän piirtämiseen. Tauri ei tarjoa selainta vaan taustalla pyörii Rust-prosessi, joka luo ikkunan käyttäen TAO-ohjelmistokirjastoa. Tämän jälkeen WRY-ohjelmistokirjasto luo WebView-elementin, joka kutsuu käyttöjärjestelmän tarjoamaa selainta näyttämään kuvan \ref{fig:demo_interface} mukaisen käyttöliittymän \cite{tauri-app}.
    ...
    Demosovellus alkaa \lstinline{main.rs}-tiedoston \lstinline{fn main()}-funktiosta.
...
.map_err(|e| LogErrorMessage::ConnectionError(e.to_string()))
    ...
    Tuloksista voidaan myös todeta, että \lstinline{LinkedList} tietorakennetta tulisi käyttää vain harvoissa tilanteissa. Rust-kielen standardikirjaston dokumentaatio vahvistaa tämän \cite[\textit{LinkedList}]{rust-std}. Kääntäjä pystyy tekemään erinomaisia optimointeja näille muille tietorakenteille ja tietokoneen prosessori pystyy hyödyntämään välimuistia tehokkaammin.
...
verkon aiheuttamaa häiriötä.
    ...
    Editorina pääsääntöisesti oli käytössä \textit{RustRover}, jossa oli muutama tärkeä lisäosa, kuten \textit{IdeaVim}, \textit{Grazie Pro}, \textit{SonarLint} ja \textit{WakaTime}.
    ...
    tarkkoja TypeScript sääntöjä ja TypeShare-kirjastoa, jotta Rust-tyypit saatiin vastaamaan täsmälleen TypeScript-tyyppejä.\par
