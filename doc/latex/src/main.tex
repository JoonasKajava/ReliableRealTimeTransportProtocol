...
\begin{document}
    ...
    \par Työ on luonteeltaan tutkiva ja ohjeistava, jonka selventää reaaliaikaisien sovelluksien toteutusta.
    ...
    Tässä dokumentissa käydään läpi internet protokollien teoriaa, toteutukset käyttäen rust-ohjelmointikieltä ja lopuksi tutkiva osuus jossa mitataan protokollien suorituskykyä ja tehokkuutta.
    ...
    \par
    UDP:n etu realiaikaisessa kommunikaatiossa on, että se on kevyt ja nopea. Se ei kuitenkaan tarjoa kattavaa virheenkorjausta, järjestystä tai ruuhkanhallintamekanismeja. Tarkistussumma on ainut virheentarkistus, minkä UDP tarjoaa. Tarkistussumma ei kuitenkaan yksin riitä vakaaseen yhteyteen. UDP sisältä
...
pub struct Frame {
    ...
    \lstinline{[u8; MAX_FRAME_SIZE]} on taulukkotyyppi joka koostuu tavuista ja on kooltaan yhtä suuri kuin $F_{max}$.
   ...
   \end{framed} 
    ...
    \subsection{Tietojenkäsittely paketissa}
...
pub fn set_sequence_number(&mut self, sequence_number: u32) {
    ...
    Oikea tavujärjestys on erittäin tärkeä osa tiedon siirrossa verkon ylitse. Protokollaa käyttävät tietokoneet täytyvät olla yhteisymmärryksessä mitä tavujärjestystä tiedonsiirrossa käytetään. laskeva tavujärjestys on yleisin järjestys, joten päätin käyttää sitä myös tässä protokollassa. \par
...
.map_err(|e| LogErrorMessage::ConnectionError(e.to_string()))
    ...
    Mittauksessa mitattiin tavallisen taulukon, kasa varattu taulukon,
    ...
    Taulukon \ref{tab:collection_bench} tuloksien perusteella kasataulukko olisi paras vaihtoehto näistä. Tästä huolimatta projektissa käytettiin \lstinline{Vec} tietorakennetta, sillä taulukon koko ei ole tiedossa kääntämisen aikana.

    \begin{framed}
        Tuloksista voidaan myös todeta, että \lstinline{LinkedList} tietorakennetta ei tulisi käyttää melkein koskaan. Rust-kielen standardi-kirjaston dokumentaatio vahvistaa tämän \cite{rust-linked-list}. Kääntäjä pystyy tekemään erinomaisia optimointeja näille muille tietorakenteille ja CPU pystyy hyödyntämään välimuistia tehokkaammin.
    \end{framed}


    \subsection{Tiedostonsiirtäminen}
...
Tiedonsiirron testaamisessa käytin 30Mt kokoista videotiedostoa, jotta 
...
tiedonsiirto kykyjä käyttäen silmukkaosoitetta. \par
Kuvasta \ref{fig:performance} voidaan päätellä, että 
\textit{lib\_rrtp}:n suorituskyky on kohtalaisen hyvä verrattuna \textit{UDP}:n ja \textit{TCP}:n. \textit{TCP} on näistä selvästi nopein, joka johtunee siitä että
puskurin käsittely on lähes automaattinen ja puskuri kasvaa tarvittaessa. 
\textit{UDP}:n ja \textit{lib\_rrtp}:n kanssa käytettiin 128 tavun kokoista puskuria, joka selittää näiden kahden tuloksen hitauden. Tuloksien mukaan \textit{UDP} on 
hidas lähetyksen alussa mutta nopeutuu ajanmyötä. Aikarajoituksien takia, en
ehtinyt selvittää olisiko tätä hitautta pystynyt korjaan jotenkin. \par

Tuloksien loppupäätelmänä pidän, että \textit{lib\_rrtp} suoriutuu suhteellisen hyvin tehtävästään, mutta huomiota täytyy kiinniittä sen tehokkuuteen kun siirretään isoja tiedostoja. Kuvan \ref{fig:performance} nähdään, että kirjaston suorituskyky pienenee mitä enemmän tietoa on siirretty. Tämä johtunee siitä, että kirjaston sisäinen puskuri täytyy ja elementtien käsittely vie enemmän aikaa.



\begin{framed}
Silmukkaosoitteen takia tämä testi ei vastaa täysin todellusuutta.
Testin tarkoitus on pääasiassa seurata tietorakenteiden käsittely tehokkuutta ilman
...
\end{framed}
    ...
    reaaliaikaisien kommunikaatio järjestelmien luontiin. Työn tuloksena syntyi \textit{lib\_RRTP}-kirjasto, joka hyödyntää UDP-protokollaa reaaliaikaiseen kommunikointiin. UDP-protokollan päälle lisättiin useita toimintoja, jotta yhteys toimii vakaasti. \par
    ...
    Koodi on jaettu yksinkertaisiin moduuleihin ja suunniteltu niin, että laajennus tai muutokset ovat helppoja toteuttaa.\par

    Kehityksen aikana projektin ulkoiset asiat aiheuttivat ajallisia ongelmia, jonka takia kehitys usein jäi aikataulusta jälkeen. Iso projekti töissä ja tiivis kurssi kehityksen aikana vei ison osan potentiaalisesta kehitysajasta. Tämän takia kaksi suunniteltua toimintoa jäi toteuttamatta. Ensimmäinen näistä oli dynaaminen aikakatkaisu paketeille ja toinen oli dynaaminen koko liukuvalle ikkunalle.
    Näille kahdelle toiminnolle on paikka lähdekoodissa, mutta toteutus jäi kesken ja testaamatta. Kirjasto toimii kuitenkin ilman näitä, mutta ei suoriudu optimaalisesti tehtävistä. \par

    Lopputuloksena kirjastosta syntyi vakaa ja toimiva ratkaisu reaaliaikaiselle kommunikaatiolle. Kirjasto kirjoitettiin täysin käyttäen turvallista rust-kieltä,
    ...
    Tätä philosofiaa pystyttiin myös jatkamaan käyttöliittymän puolelle käyttämällä
    tarkkoja TypeScript sääntöjä sekä TypeShare-kirjastoa, jotta rust mallit saatiin vastaamaan täsmälleen typescipt malleja.\par

    Protokollan kehityksessä opittiin miten säikein hallinta tulee toteuttaa ja miten rust-kieli auttaa tässä. Tärkeänä osana oli myös toteuttaa toimiva käyttöliittymä, joka pystyy hyödyntämään rust-kielen vahvuuksia. Tämä on oli erityisen tärkeää sillä
    uskon vahvasti, että rust-kieli on tulevaisuuden kieli. Se on löytänyt tiensä Linux ja Windows kerneliin. Rust-kielen turvallisuus, tehokkuus ja modernit ominaisuudet tekevät siitä lähes täydellisen kielen, jolla on mahdollisuus korvata c ja c++ tulevaisuudessa.
